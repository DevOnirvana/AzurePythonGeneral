...
def doHandleEvent(scope, resource):
    '''This function calls the class with arguments extracted from event b and decide whether the remediation to be performed or not '''
    try:
...
class ProjectventHandlerAzureStorageAccountAccess(ProjectEventHandler.ProjectEventHandlerAzureService):
    ...
    def isRuleViolated(self):
        '''This functions checks whether the storage account permits requests to be authorized with the account access key via Shared Key, If yes
        ...
        try:
            ...
            if data['properties']['allowSharedKeyAccess'] is True:
                return True
            else:
    ...
    def remediate(self):
        '''This function disables that the storage account permits requests to be authorized with the account access key via Shared Key'''
        try:
